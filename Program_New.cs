    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length == 2 
                ...
                && (args[1] == "."))
            {
                var solutionDirs = collectDirectories(Environment.CurrentDirectory);
                if (solutionDirs.Any())
                {
                    foreach (var dir in solutionDirs)
                    {
                        cleanSolutions(new string[] { dir });
                    }
                }
            }
            else
        ...
        static void cleanSolutions(string[] args)
        {
        ...
        private static List<string> collectDirectories(string rootPath)
        {
            if (Directory.Exists(rootPath))
            {
                var files = Directory.EnumerateFiles(rootPath, "*sln", SearchOption.AllDirectories);
                var dirPaths = files.Select(f => new FileInfo(f)).Select(f => f.DirectoryName).ToList();
                return dirPaths;
            }
